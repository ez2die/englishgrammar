...
import { SentenceAnalysisData, SentenceStructure, GrammarRole, DifficultyLevel } from "../types";
import { SAMPLE_DATA_FALLBACK } from "../constants";
import { validatePrepositionalPhrases, postProcessRoles } from "./validationService";
...
export const generateSentenceAnalysis = async (level: DifficultyLevel): Promise<SentenceAnalysisData> => {
  if (!apiKey) {
    ...
    return { ...SAMPLE_DATA_FALLBACK, level } as any;
  }
  ...
  try {
    ...
    const normalizedRoles: Record<number, GrammarRole> = {};
    if (Array.isArray(data.wordRoles)) {
      data.wordRoles.forEach((role: string, index: number) => {
        normalizedRoles[index] = role as GrammarRole;
      });
    }
    ...
    let result: SentenceAnalysisData = {
      originalSentence: data.originalSentence,
      words: data.words,
      wordRoles: normalizedRoles,
      structureType: structure as SentenceStructure,
    ...
    if (!validation.isValid) {
      ...
      // Note: We still return the post-processed result even if validation fails
      // The post-processing should have fixed most issues, but some edge cases might remain
    }
